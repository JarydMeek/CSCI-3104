...
class Graph():
    ...
    def Prim(self):
        ...
        """
        #Create queue and fill with letters a-N
        queue = [x for x in range(0,self.N)]
        ...
        while queue:
            ...
            for x,y in enumerate(queue):
                if self.adjacency_matrix[current][y] < cost[x] and self.adjacency_matrix[current][y] > 0:
                    cost[x] = self.adjacency_matrix[current][y]
            #Sorts Queue (and Cost) 
            queue = [x for _,x in sorted(zip(cost,queue))]
            cost.sort()
...
'''
#  Given Test Case:
G = Graph([[0, 10, 11, 33, 60],[10, 0, 22, 14, 57],[11, 22, 0, 11, 17],[33, 14, 11, 0, 9],[60, 57, 17, 9, 0]])
print(G.Prim())
#assert G.Prim() == 41
