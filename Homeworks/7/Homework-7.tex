...
\rhead{Name: \fbox{Jaryd Meek}  \\  ID: \fbox{} \\ Collaborators: Noah Nguyen, Emily Parker \\ {\bf Due March 12, 2021 \\ Spring 2021, CU-Boulder}}
...
% STUDENTS: TYPE YOUR SOLUTIONS HERE. (Between \textbf{Solution:} and \fi )
%==============================================================================
Since the version of Dijkstra's algorithm stops when it finds the correct node, Dijkstra's algorithm on this graph will go $S\rightarrow U\rightarrow V\rightarrow T$ then stop.
Since it terminates before traversing down the lower path (since the final distance will be 3, which is less than the 4 needed to go down the lower path to $w$), it will never encounter the negative weight, causing it to pick a non-ideal path.
